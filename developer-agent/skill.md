...
- **Structure:** FastAPI monolith under `src/tvevents/` with `api/` (routes, models, ops), `domain/` (security, validation, transform, obfuscation, event_types, delivery), `infrastructure/` (database, cache), and `middleware/` (metrics).
- **Data flow:** TVs POST JSON payloads → validate HMAC + params → dispatch to event type handler → flatten/transform → obfuscate blacklisted channels → deliver to Kafka topics.
- **Service boundaries:** Single service. Standalone RDS module (`output/rebuilder-rds-module/`) and Kafka module (`output/rebuilder-kafka-module/`) are external packages.
- **Infrastructure:** AWS EKS (Kubernetes), AWS RDS PostgreSQL, AWS MSK (Kafka). Terraform in `terraform/`, Helm charts in `charts/`.
- **API:** `POST /` (ingestion), `GET /status`, `GET /health`, `/ops/*` (11 diagnostic + remediation endpoints). OpenAPI auto-generated by FastAPI.
...
- Local services: `docker compose up -d` (PostgreSQL, Kafka)
- Database setup: `psql -f scripts/seed_db.sql` (seed blacklisted channel IDs)
- Cache init: Automatic at startup via entrypoint.sh (queries RDS, writes file cache)
- Test: `pytest -v --tb=short`
- Lint: `ruff check . && ruff format --check . && mypy src/`
- Run: `uvicorn src.tvevents.main:app --reload --port 8000`
...
- **Logging:** Structured JSON to stdout via Python `logging` with OTEL trace correlation (`trace_id`, `span_id`)
- **Metrics and tracing:** OpenTelemetry auto-instrumentation via `opentelemetry-instrument` in entrypoint.sh, OTLP HTTP exporters to New Relic
