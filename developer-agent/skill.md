...
- FastAPI service with a layered directory structure: `src/tvevents/api/` (route handlers), `src/tvevents/domain/` (business logic and models), `src/tvevents/services/` (external integrations — Kafka, Redis, RDS), `src/tvevents/ops/` (SRE diagnostic and remediation endpoints), `src/tvevents/middleware/` (request lifecycle, auth, error handling).
- Data flows in via HTTP POST to the tvevents-api ingestion endpoints. Events are validated, enriched against the Redis cache and RDS blacklist, and delivered to Apache Kafka topics for downstream consumers.
- Redis (via rebuilder-redis-module) provides low-latency caching for lookups. PostgreSQL (AWS RDS via asyncpg) stores the blacklist and persistent state.
- Apache Kafka (via confluent-kafka) is the event delivery backbone. The service produces to Kafka topics; downstream consumers are separate services.
- Infrastructure runs on AWS: EKS (Kubernetes) for the container, RDS for PostgreSQL, ElastiCache for Redis, MSK for Kafka.
- API endpoints are organized under `/v1/`, documented via OpenAPI spec auto-generated by FastAPI. The spec is available at `/docs` (Swagger UI) and `/openapi.json`.
...
- `pip install -e ".[dev]"` — install all dependencies including dev extras
- `docker compose up` — start PostgreSQL, Redis, and Kafka locally
- `uvicorn tvevents.main:app --reload --port 8000` — run dev server with hot reload
...
- **Logging:** Structured JSON to stdout via Python `logging` module, with OTEL trace ID and span ID correlation fields injected into every log record for end-to-end request tracing.
- **Metrics and tracing:** OpenTelemetry SDK for distributed tracing and metrics collection, exported via OTLP to an OpenTelemetry Collector sidecar. Traces propagate W3C TraceContext headers across service boundaries.
