...
- FastAPI application factory pattern in `src/app/__init__.py` with asynccontextmanager lifespan; uvicorn entry point in `src/app/main.py`
- Request flow: POST `/` → request validation (T1_SALT HMAC via cnlib.token_hash) → event type classification (NativeAppTelemetry, AcrTunerData, PlatformTelemetry) → output JSON generation with payload flattening → channel obfuscation (blacklist check) → Kafka delivery (via standalone Kafka module)
- Standalone RDS module (external pip package) for PostgreSQL blacklist queries with connection pooling; standalone Kafka module (external pip package) for event delivery with topic routing
- 3-tier blacklist cache: memory dict → JSON file (`BLACKLIST_CHANNEL_IDS_CACHE_FILEPATH`) → RDS query via standalone module. File cache path configurable via environment variable.
- Helm charts for EKS deployment with KEDA autoscaling (1–500 replicas). Dockerfile follows template-repo-python pattern (Python 3.12-bookworm, non-root containeruser UID 10000, port 8000).
...
- **Install:** `pip install -e ".[dev]"` or `pip install -r requirements.txt && pip install -r requirements-dev.txt`
- **Local services:** `docker compose up -d` (starts PostgreSQL and Kafka for local development)
- **Database setup:** N/A — read-only access to shared RDS blacklist table. Local PostgreSQL seeded via docker-compose.
- **Seed data:** N/A — blacklist cache auto-populates from database on startup
- **Test:** `pytest tests/ --cov=src/app --cov-fail-under=80`
...
- **Logging:** Structured JSON to stdout via cnlib.log
- **Metrics and tracing:** OpenTelemetry auto-instrumentation (FastAPIInstrumentor, Psycopg2Instrumentor, URLLib3Instrumentor) with OTLP exporter to New Relic
