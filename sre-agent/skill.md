...
- **Additional:** Apache Kafka (confluent-kafka), OpenTelemetry → OTEL Collector
...
3. If degraded/unhealthy → call GET `/ops/health`, GET `/ops/dependencies`, GET `/ops/errors`, GET `/ops/metrics`, GET `/ops/config`
...
- PUT `/ops/loglevel` — adjust verbosity
